...
void SceneLight::Update(float tick)
{
	...
	angle += 0.5f * tick; // rotation speed
	...
	for (int i = 0; i < 30; ++i)
	{
		...
		DirectX::XMFLOAT3 pos = pPointLight->GetPos();

		float radius = sqrtf(pos.x * pos.x + pos.z * pos.z);
		if (radius < 0.001f)
			radius = 1.0f; // avoid zero radius

		float baseAngle = atan2f(pos.z, pos.x);

		float newAngle = baseAngle + 0.5f * tick;

		pos.x = cosf(newAngle) * radius;
		pos.z = sinf(newAngle) * radius;

		pPointLight->SetPos(pos);
	}
}
